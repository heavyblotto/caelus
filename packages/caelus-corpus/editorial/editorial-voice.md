...
## Product copy (Caelus Free)

Consumer copy in the Caelus Free app addresses a person who wants astrology
and has never heard of an ephemeris. Every rule above applies, and four
more:

- **Write from the user's side.** Every sentence says what the user gets or
  does: "You don't need an account." "Your birth data never leaves your
  device."
- **Keep insider vocabulary out.** Engine and trade terms stay off consumer
  surfaces: Caelus (outside the product name Caelus Free), validated,
  provenance, fact atom, dark pattern, canonical, MCP. When the concept has
  to surface, translate it: "see where this sentence comes from" carries
  what "citation" means.
- **Plain spoken sentences.** A sentence that would sound odd said out loud
  to a friend gets rewritten.
- **Vary the rhythm.** Runs of short clipped sentences and lists of
  parallel imperatives read as machine cadence. Mix sentence lengths and
  connect them the way conversation does.

## For agents

1. Read this file before editing any user-facing prose: app copy, docs, or
   package READMEs. For corpus entries, read `pipeline/voice-sheet.md` too;
   it extends these rules with what delineation prose additionally owes.
