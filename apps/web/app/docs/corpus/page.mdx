...
The Playground **Reading** tab runs the unpublished in-repo `caelus-corpus`
(original essays against the engine's fact atoms). **`caelus-delineations-pd`**
remains the published npm reference and validation set.
...
the only complete natal planet-in-sign source whose OCR is usable; they
...
374 passages across eight active sources: planet-in-sign (Sun through Neptune),
planet-in-house (Alan Leo), planet-aspect-planet and planet-parallel-planet
(Heindel), rising sign (Heindel), fixed-star conjunctions (Robson, curated),
and Moon-in-nakshatra (Brihat Jataka ch. XVI, all 27). Dignity and lot cells
have selectors but no PD passages yet. See the package README for the full
table and known gaps.
