...
Caelus is a fact engine, and every module in it stops at validated geometry. A
chart gives you bodies, aspects, angles, cusps, dignities, and patterns, with no
delineation text and no account of what any of it means. That boundary is
deliberate, because the numbers can be made correct and auditable while the
meaning of a chart remains a matter of tradition and taste rather than
arithmetic.

That boundary does leave a gap. If you want to *generate* an interpretation,
whether from a rule base or from an LLM writing natal prose, there is no clean
place to attach it. The interpretation layer exists to be that attachment
point, and it performs no interpretation of its own. What it defines is the
contract between the facts the engine owns and the meaning you bring to them:

> The engine owns the **facts**, a layer you plug in owns the **meaning**, and
> the framework owns the **contract** between the two rather than any of the
> content itself.
...
**Degree** atoms state each point's zodiacal degree ordinally, the way degree
symbols count: a body at 14°30′ Aries occupies the *15th* degree of Aries.
One atom per placed body plus the Ascendant and Midheaven, each also carrying
the ten-degree face index (1-3). Match with
`hasDegree({ sign: "Aries", degree: 15 })`.
...
**Parallel** and **out-of-bounds** atoms are declination facts, which longitude
cannot express: two bodies can hold a parallel while unaspected
(`parallel:mars~venus:parallel`), and a body can run beyond the obliquity band
(`oob:moon`). Match them with `hasParallel({ between: ["mars", "venus"] })` and
`hasOutOfBounds({ body: "moon" })`.
...
- **`enrichSynastryOptions(engine, chartA, chartB)`**: inter-chart aspects,
  house overlays, composite midpoint placements, and the aspects among those
  composite bodies (chart A is the base). Composite placements also carry a
  **composite house** when both charts have real angles: a midpoint composite
  has no moment and no place to cast houses from, so `compositeFrame` supplies
  a stated convention instead (midpoint Ascendant and MC, equal houses from
  the Ascendant, `method: "equal-from-midpoint-asc"`). Without both birth
  times there is no frame and no house, rather than an invented one, and
  `hasComposite({ house })` correctly matches nothing. Note the composite MC
  does not generally land on the tenth cusp: two independently averaged angles
...
### Facts the chart alone does not state

Some facts a reader takes for granted are not on a `Chart`: a planet sitting on
the Ascendant is not an aspect, a body's nearness to the Sun is not a dignity,
and a return is a relation between two instants. The projection computes these
itself, so they arrive as atoms with ids like any other fact.

- **`angleContact`** (`angleContact:sun:asc`): a body conjunct the ASC, DSC,
  MC, or IC point, not the angular *house*. Orb via `angleOrb` (default 8°).
  Match with **`hasAngleContact`**.
- **`transitHouse`** (`transitHouse:saturn:7`): the "Saturn is crossing your
  7th" fact, true whether or not anything is in orb. **`hasTransitHouse`**.
- **`station`** (`station:mercury:retrograde`): a body turning near the
  instant, inside an editorial window (default ±5 days). **`hasStation`**.
- **`return`** (`return:saturn:1`): a body within orb of its own natal
  longitude, numbered, so a first Saturn return is addressable apart from a
  second. **`hasReturn`** takes `nth` or `minNth`.
- **`lunation`** (`lunation:full:eclipse:lunar`): a New or Full Moon near the
  instant, located in the natal houses, eclipse-flagged, listing any natal
  bodies it falls on. **`hasLunation`** filters phase, eclipse, house, and
  `onNatal`.
- **`solarPhase`** (`solarPhase:mercury:cazimi`): cazimi, combust, or under
  the beams, at the pinned electional thresholds. Fires on a birth chart and on
  the moving sky alike. **`hasSolarPhase`**.
- **`parallel`** and **`outOfBounds`** (`parallel:mars~venus:parallel`,
  `oob:moon`): declination facts, which longitude cannot express: two bodies
  can hold a parallel while unaspected. **`hasParallel`**, **`hasOutOfBounds`**.
- **`compositeAspect`** (`compositeAspect:mars~venus:square`): an aspect
  between two *composite* placements. A composite chart is a set of midpoints,
  so its internal geometry is a fact about the relationship itself rather than
  about either person; `composite` atoms give the placements, this gives the
  relations among them. **`hasCompositeAspect`**.

Three of these close gaps where the engine's own primitives deliberately look
away: `findAspects` excludes the nodes, so the projection computes node aspects
itself; `transitAspects` admits one natal node as a *target*; and a classical
planet with none of the five essential dignities is marked `"peregrine"` on its
placement atom, while `Chart.dignities` stays exactly as computed.
...
| `synastry` / `composite` | an inter-chart aspect or overlay / a composite midpoint placement or composite aspect |
...
Writing a **complete** `SalienceWeights` object literal requires `planetReturn`,
`lunation`, and `solarPhase` as of 0.25.0; `Partial<>` overrides through
`ContextOptions.salience` are unchanged.
...
<CodeBlock
  ...
  hasDispositor, hasReception, hasStar, hasLot, hasDegree,
...
hasDegree({ sign: "Aries", degree: 15 });         // any point in the 15th degree of Aries
