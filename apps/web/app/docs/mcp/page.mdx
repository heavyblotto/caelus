...
There are two ways to connect, with the same fourteen tools either way:
...
After the client reloads, it should list **fourteen** tools. Ask it to enumerate
...
and sect for the seven traditional planets), and `lots` (the seven Hermetic
lots, sect-aware).
...
If you see all fourteen, the server is wired correctly. The server also exposes two
