"use client";
/**
 * Client half of a sect-flip plate. Same contract as the other
 * plates: the server rendered the article's scene; hydration attaches
 * the day/night picker beneath it, and a reader's chart swaps in a
 * browser-computed scene. The embedded data tier loads only when
 * needed.
 */
import { useEffect, useState } from "react";
import type { Engine } from "caelus";
import { SectFlip } from "caelus-widgets/sect-flip-widget";
import type { SectScene } from "caelus-widgets/sect-flip";
import { useReaderChart } from "../ReaderChartContext";

let enginePromise: Promise<Engine> | null = null;
function browserEngine(): Promise<Engine> {
  return (enginePromise ??= Promise.all([
    import("caelus"),
    import("caelus/data-embedded"),
  ]).then(([{ Engine: E }, { embeddedData }]) => new E(embeddedData)));
}

export default function SectFlipPlate({
  scene, size,
}: { scene: SectScene; size?: number }) {
  const { params } = useReaderChart();
  const [override, setOverride] = useState<SectScene | null>(null);

  useEffect(() => {
    if (!params) {
      setOverride(null);
      return;
    }
    let stale = false;
    Promise.all([browserEngine(), import("caelus-widgets/sect-flip")])
      .then(([engine, { deriveSect }]) => {
        const s = deriveSect(engine, {
          ...scene.params,
          instant: params.instant,
          place: params.place,
          houseSystem: params.houseSystem ?? scene.params.houseSystem,
        });
        if (!stale) setOverride(s);
      })
      .catch(() => {
        if (!stale) setOverride(null);
      });
    return () => {
      stale = true;
    };
  }, [params, scene]);

  return <SectFlip scene={override ?? scene} size={size} />;
}
